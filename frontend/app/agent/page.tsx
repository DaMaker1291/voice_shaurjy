...
export default function AgentPage() {
  ...
  const statusColor = (s: string) =>
    s === "completed" ? "bg-[#34d399]" : s === "dispatched" ? "bg-[#f59e0b]" : s === "pending" ? "bg-[#a78bfa]" : "bg-gray-600";
      ...
      <div className="page-ambient flex-1 overflow-y-auto">
        <div className="max-w-6xl mx-auto p-4 sm:p-6">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-3">
            <div>
              <h1 className="text-lg font-bold tracking-wider text-[#a78bfa]/80">JARVIS Agent Control</h1>
              {status && (
            ...
            <div className="flex items-center gap-2 flex-wrap">
              {quickCmds.map(c => (
                <button
                  ...
                  onClick={() => doIssue(c)}
                  disabled={issuing === c}
                  className="text-[9px] font-mono tracking-wider uppercase px-3 py-1.5 rounded-lg border border-[#a78bfa]/20 text-[#a78bfa]/60 hover:text-[#a78bfa] hover:border-[#a78bfa]/40 transition-all disabled:opacity-40"
                >
                  {issuing === c ? "..." : c}
                </button>
              ))}
          ...
          <div className="flex gap-0.5 mb-6 border-b border-gray-800/30 overflow-x-auto">
            {tabs.map(t => (
              <button
                key={t.id}
                onClick={() => setTab(t.id)}
                className={`px-4 py-2 text-[10px] font-mono tracking-wider transition-all whitespace-nowrap ${
                  tab === t.id
          ...
          {tab === "scan" && (
            <div>
              <button
                ...
                className="mb-4 text-[9px] font-mono tracking-wider uppercase px-4 py-2 rounded-lg border border-[#a78bfa]/20 text-[#a78bfa]/60 hover:text-[#a78bfa] hover:border-[#a78bfa]/40 transition-all disabled:opacity-40"
              >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {scanData.wifi && (
                    <div className="glass-card p-4">
                      <h3 className="text-[9px] font-mono text-[#a78bfa]/60 tracking-[0.2em] uppercase mb-2">WiFi</h3>
                  ...
                  {scanData.lan && (
                    <div className="glass-card p-4">
                      <h3 className="text-[9px] font-mono text-[#a78bfa]/60 tracking-[0.2em] uppercase mb-2">LAN Devices ({scanData.lan.count || (scanData.lan.devices || []).length})</h3>
                  ...
                  {scanData.processes && (
                    <div className="glass-card p-4">
                      <h3 className="text-[9px] font-mono text-[#a78bfa]/60 tracking-[0.2em] uppercase mb-2">Top Processes</h3>
                      <div className="space-y-1 max-h-40 overflow-y-auto">
                  ...
                  {scanData.users && (
                    <div className="glass-card p-4">
                      <h3 className="text-[9px] font-mono text-[#a78bfa]/60 tracking-[0.2em] uppercase mb-2">Users</h3>
          ...
          {tab === "propagation" && (
            <div>
              ...
              {propData ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <div className="glass-card p-4">
                    <h3 className="text-[9px] font-mono text-[#a78bfa]/60 tracking-[0.2em] uppercase mb-2">Status</h3>
                  ...
                  <div className="glass-card p-4">
                    <h3 className="text-[9px] font-mono text-[#a78bfa]/60 tracking-[0.2em] uppercase mb-2">Logs ({propLogs.length})</h3>
                    <div className="space-y-1 max-h-60 overflow-y-auto">
                      {(propLogs || []).slice().reverse().map((l: any, i: number) => (
                        <div key={i} className="text-[8px] font-mono text-gray-600 leading-tight flex gap-2">
                          ...
                          <span className={l.level === "error" ? "text-[#ef4444]/60" : "text-gray-600"}>{l.message || JSON.stringify(l)}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  {shDevices.map((d: any, i: number) => (
                    <div key={i} className="glass-card p-4">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <span className={`w-2 h-2 rounded-full ${d.status === "on" || d.status === "online" ? "bg-[#34d399]" : "bg-gray-600"}`} />
                      </div>
