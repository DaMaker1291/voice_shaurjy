          ...
          {platforms.map((p) => (
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg text-[10px] font-mono transition-all ${
                platform === p.key
                  ? "bg-purple-600/20 text-purple-300 border border-purple-700/30 shadow-sm"
          ...
          <div className="bg-gray-900 rounded-xl p-3 text-xs text-gray-300 break-all select-all relative group font-mono border border-purple-800/20">
            <div className="flex items-center justify-between mb-1.5">
              ...
              <button onClick={() => copyCmd(cur.cmd, copyKey)}
                className={`text-[9px] font-mono px-2 py-1 rounded-lg transition-all ${isCopied ? "bg-green-900/30 text-green-400" : "bg-gray-800 text-gray-500 hover:text-gray-300 hover:bg-gray-700"}`}>
                {isCopied ? "✓ Copied" : "Copy"}
              </button>
            </div>
            <code className="text-[11px] leading-relaxed text-purple-200/80">{cur.cmd}</code>
          </div>
        </div>
