      ...
      <div className="page-ambient flex-1 overflow-y-auto">
        <div className="max-w-3xl mx-auto p-4 sm:p-6 space-y-6">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-2xl font-bold text-[#a78bfa]">Reminders</h1>
          ...
          <section className="glass-card p-5 space-y-3">
            ...
            <div className="flex flex-col gap-2">
              <input
                ...
                className="bg-gray-800/80 border border-gray-700 rounded-lg px-4 py-2.5 text-sm text-gray-200 placeholder-gray-600 outline-none focus:border-[#a78bfa]/50"
              />
              <div className="flex flex-col sm:flex-row gap-2">
                <input
                  ...
                  className="flex-1 bg-gray-800/80 border border-gray-700 rounded-lg px-4 py-2 text-sm text-gray-200 placeholder-gray-600 outline-none focus:border-[#a78bfa]/50"
                />
                <input
                  type="date"
                  value={due}
                  onChange={(e) => setDue(e.target.value)}
                  className="bg-gray-800/80 border border-gray-700 rounded-lg px-4 py-2 text-sm text-gray-200 outline-none focus:border-[#a78bfa]/50"
                />
                <button
                  onClick={handleAdd}
                  className="px-4 py-2 bg-[#a78bfa]/20 border border-[#a78bfa]/30 rounded-lg text-sm text-purple-300 hover:bg-[#a78bfa]/30 transition-colors"
                >
                  Add
                </button>
              </div>
              ...
              {active.map((r) => (
                <div key={r.id} className="glass-card p-4 flex items-center gap-3">
                  <button
                    ...
                    className="w-5 h-5 rounded-full border-2 border-gray-600 hover:border-[#a78bfa] transition-colors flex-shrink-0"
                  />
                  ...
                  <button
                    ...
                    className="text-gray-600 hover:text-[#ef4444] transition-colors flex-shrink-0"
                  >
          ...
          {done.length > 0 && (
            <section className="space-y-2">
              ...
              {done.map((r) => (
                <div key={r.id} className="glass-card p-4 flex items-center gap-3 opacity-50">
                  <button
                    onClick={() => handleToggle(r.id, r.completed)}
                    className="w-5 h-5 rounded-full border-2 border-[#34d399]/70 bg-[#34d399]/30 flex-shrink-0"
                  >
                    <svg className="w-3 h-3 text-[#34d399] mx-auto mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  ...
                  <button
                    onClick={() => handleDelete(r.id)}
                    className="text-gray-700 hover:text-[#ef4444] transition-colors flex-shrink-0"
                  >
                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                    </svg>
