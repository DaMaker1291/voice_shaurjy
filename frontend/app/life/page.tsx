...
import {
  ...
  addLifeTransaction, logLifeWorkout, logLifeMeal, logLifeSleep, logLifeWater,
  addLifeTask, getLifeMorningBriefing,
} from "@/lib/api";
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [d, f, b, s, h, p, hab, g, j, m, br] = await Promise.allSettled([
        ...
        getLifeJournal(), getLifeMoodTrend(), getLifeMorningBriefing(),
      ]);
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div><h1 className="text-xl font-bold text-purple-400">Life OS</h1><p className="text-xs text-gray-500 mt-0.5">Complete life operating system — finance, health, tasks, goals</p></div>
        {(finance || health) && (
      ...
      <div className="flex gap-1 border-b border-gray-800/30 pb-2 overflow-x-auto">
        {(["dashboard", "finance", "health", "planner", "habits", "journal"] as Tab[]).map((t) => (
          <button key={t} onClick={() => setActiveTab(t)} className={`px-4 py-1.5 text-xs font-mono rounded-t-lg transition-colors whitespace-nowrap ${activeTab === t ? "text-purple-400 bg-purple-900/10 border-b-2 border-purple-500" : "text-gray-600 hover:text-gray-400"}`}>
            {t === "dashboard" ? "Dashboard" : t === "finance" ? "Finance" : t === "health" ? "Health" : t === "planner" ? "Planner" : t === "habits" ? "Habits & Goals" : "Journal"}
          </button>
        ))}
      </div>
      {loading ? (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3"><SkeletonCard lines={1} /><SkeletonCard lines={1} /><SkeletonCard lines={1} /><SkeletonCard lines={1} /></div>
      ) : (
        <>
          {activeTab === "dashboard" && (
            <div className="space-y-4">
              {briefing && (
                <div className="bg-purple-900/10 border border-purple-800/20 rounded-xl p-5">
                  <p className="text-[10px] font-mono text-purple-400/60 uppercase tracking-wider mb-2">Morning Briefing</p>
          ...
          {activeTab === "finance" && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => setShowAddTxn(!showAddTxn)} className="text-xs text-purple-400 hover:text-purple-300">+ Add Transaction</button>
              </div>
              {showAddTxn && (
                <div className="bg-gray-900/60 border border-gray-800/50 rounded-xl p-5 space-y-3">
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div className="col-span-2"><label className="text-[10px] text-gray-500 font-mono">Description</label><input value={txnDesc} onChange={e => setTxnDesc(e.target.value)} className="w-full bg-gray-800/50 border border-gray-700/50 rounded px-3 py-1.5 text-xs text-gray-200 mt-1" /></div>
                    <div><label className="text-[10px] text-gray-500 font-mono">Type</label><div className="flex gap-2 mt-1"><button onClick={() => setTxnType("expense")} className={`px-3 py-1.5 text-xs rounded-lg ${txnType === "expense" ? "bg-red-900/40 text-red-400" : "bg-gray-800/50 text-gray-500"}`}>Expense</button><button onClick={() => setTxnType("income")} className={`px-3 py-1.5 text-xs rounded-lg ${txnType === "income" ? "bg-green-900/40 text-green-400" : "bg-gray-800/50 text-gray-500"}`}>Income</button></div></div>
                  </div>
                  <button onClick={handleAddTxn} className="px-4 py-2 bg-purple-600 hover:bg-purple-500 text-white text-xs rounded-lg transition-colors">Save</button>
                </div>
              )}
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-gray-900/40 border border-gray-800/30 rounded-xl p-5"><p className="text-[10px] font-mono text-gray-500 uppercase tracking-wider mb-1">Balance</p><p className="text-2xl font-bold text-green-400">${finance?.balance || 0}</p></div>
                <div className="bg-gray-900/40 border border-gray-800/30 rounded-xl p-5"><p className="text-[10px] font-mono text-gray-500 uppercase tracking-wider mb-1">Income</p><p className="text-2xl font-bold text-gray-200">${finance?.income || 0}</p></div>
                <div className="bg-gray-900/40 border border-gray-800/30 rounded-xl p-5"><p className="text-[10px] font-mono text-gray-500 uppercase tracking-wider mb-1">Expenses</p><p className="text-2xl font-bold text-red-400">${finance?.expenses || 0}</p></div>
              </div>
                ...
                <div className="bg-gray-900/40 border border-gray-800/30 rounded-xl p-5">
                  ...
                  {budgets.map((b: LifeBudget, i: number) => (
                    <div key={i} className="flex items-center justify-between py-1.5 text-xs border-b border-gray-800/20 last:border-0">
                      ...
                      <span className={`font-mono ${b.status === "over" ? "text-red-400" : b.status === "warning" ? "text-yellow-400" : "text-gray-500"}`}>${b.spent} / ${b.budget} ({b.pct_used}%)</span>
                    </div>
                  ))}
                </div>
              )}
          ...
          {activeTab === "planner" && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => setShowAddTask(!showAddTask)} className="text-xs text-purple-400 hover:text-purple-300">+ Add Task</button>
              </div>
              {showAddTask && (
                <div className="bg-gray-900/60 border border-gray-800/50 rounded-xl p-5 space-y-3">
                  <div className="grid grid-cols-2 gap-3">
                    <div className="col-span-2"><label className="text-[10px] text-gray-500 font-mono">Task</label><input value={taskTitle} onChange={e => setTaskTitle(e.target.value)} className="w-full bg-gray-800/50 border border-gray-700/50 rounded px-3 py-1.5 text-xs text-gray-200 mt-1" /></div>
                  ...
                  <button onClick={handleAddTask} className="px-4 py-2 bg-purple-600 hover:bg-purple-500 text-white text-xs rounded-lg transition-colors">Add Task</button>
                </div>
              )}
                  ...
                  {planner!.pending!.map((t: any, i: number) => (
                    <div key={i} className="bg-gray-900/40 border border-gray-800/30 rounded-xl p-4 flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <div className={`w-2 h-2 rounded-full ${t.priority >= 4 ? "bg-red-500" : t.priority >= 3 ? "bg-yellow-500" : "bg-gray-600"}`} />
                ...
                <div className="space-y-2">
                  {habits.map((h: LifeHabit, i: number) => (
                    <div key={i} className="bg-gray-900/40 border border-gray-800/30 rounded-xl p-4 flex items-center justify-between">
                      ...
                      <span className={`px-2 py-0.5 text-[10px] rounded-full ${h.done_today ? "bg-green-900/30 text-green-400" : "bg-gray-800/50 text-gray-600"}`}>
              ...
              {goals.length > 0 && (
                <div className="mt-6">
                  ...
                  {goals.map((g: any, i: number) => (
                    <div key={i} className="bg-gray-900/40 border border-gray-800/30 rounded-xl p-4 mb-2">
                      ...
                      <div className="w-full h-1.5 rounded-full bg-gray-800 overflow-hidden">
                        <div className="h-full rounded-full bg-gradient-to-r from-purple-500 to-cyan-400 transition-all" style={{ width: `${g.progress}%` }} />
                      </div>
                    </div>
                  ))}
                </div>
          ...
          {activeTab === "journal" && (
            <div className="space-y-4">
              ...
              <div className="bg-gray-900/60 border border-gray-800/50 rounded-xl p-5 space-y-3">
                ...
                <button onClick={handleJournalSubmit} className="px-4 py-2 bg-purple-600 hover:bg-purple-500 text-white text-xs rounded-lg transition-colors">Save Entry</button>
              </div>
              {mood && (
                <div className="bg-gray-900/40 border border-gray-800/30 rounded-xl p-5">
