...
# ── Evidence & Mission Audit API ──────────────────────────────────────

@app.get("/api/evidence/{mission_id}")
async def get_evidence(mission_id: str):
    """Get the full evidence ledger for a mission (hash-chained audit trail)."""
    from evidence_ledger import get_evidence_ledger
    ledger = get_evidence_ledger(mission_id)
    return ledger.to_dict()


@app.get("/api/evidence/{mission_id}/timeline")
async def get_evidence_timeline(mission_id: str, limit: int = 50):
    """Get the mission timeline for UI display."""
    from evidence_ledger import get_evidence_ledger
    ledger = get_evidence_ledger(mission_id)
    return {"timeline": ledger.get_timeline(limit), "stats": ledger.get_stats()}


@app.get("/api/evidence/{mission_id}/verify")
async def verify_evidence_chain(mission_id: str):
    ...
    return {
        ...
        "chain_hash": ledger._chain_hash,
        "chain_length": ledger._sequence,
    }
...
async def list_missions():
    ...
    missions = []
    for m in engine.list_missions():
        d = m.to_dict()
        d["progress"] = m.compute_progress()
        missions.append(d)
...
async def do_anything(request: Request):
    """JARVIS OS — Universal Autonomous Execution.

    The vision pipeline:
      USER GOAL → INTENT → MISSION → WORKFORCE → EXECUTE → VERIFY → EVIDENCE

    Every execution creates an auditable evidence trail with hash-chain integrity.
    """
    body = await request.json()
    ...
    _bare = _strip_re.sub("", goal.lower().strip()).strip()
    ...
    if GREETING_RE.match(goal.lower().strip()):
    ...
    # ── Chat fast-path: pure conversational, no action intent ──
    _ACTION_KEYWORDS = [
        ...
        "organize", "sort", "rename", "move", "prepare", "build",
        "generate", "export", "convert", "download",
    ]
    _has_action = any(kw in _bare for kw in _ACTION_KEYWORDS)
    is_chat = not _has_action and (len(goal.split()) < 8 or any(goal.lower().startswith(w) for w in [
    ...
    # ═══════════════════════════════════════════════════════════════
    #  JARVIS OS PIPELINE: Intent → Mission → Workforce → Execute → Verify → Evidence
    # ═══════════════════════════════════════════════════════════════

    import time as _time
    _start = _time.time()

    # ── 1. INTENT ENGINE: Parse the goal ──
    intent_result = {"goal": goal, "intent": "action", "confidence": 0.8}
    try:
        from edge_router import classify_intent
        intent, confidence, sub_type = classify_intent(goal)
        intent_result = {"goal": goal, "intent": intent, "confidence": confidence, "sub_type": sub_type}
        if confidence > 0.95:
            safety = safety or "full_auto"
    except Exception:
        pass

    # ── 2. GOVERNANCE: Check policies before execution ──
    try:
        from policies import get_permission_manager
        pm = get_permission_manager()
        decision = pm.evaluate(goal)
        if decision and hasattr(decision, 'action') and decision.action == "deny":
            return JSONResponse({
                "success": False,
                "error": f"Blocked by policy: {decision.reason if hasattr(decision, 'reason') else 'policy violation'}",
                "pipeline": "governance",
            }, status_code=403)
    except Exception:
        pass
    ...
    if complexity.value in ("complex", "autonomous"):
        ...
        try:
            from mission_engine import get_mission_engine
            engine = get_mission_engine()
            graph = engine.create_mission(goal, workspace_id="default")
            engine.plan_mission(graph.id)
            engine.start_mission(graph.id)

            # Poll until complete (with timeout)
            import asyncio
            _timeout = 120  # seconds
            _poll_start = _time.time()
            while _time.time() - _poll_start < _timeout:
                g = engine.get_mission(graph.id)
                if g and g.status in ("completed", "failed", "stopped"):
                    break
            ...
            if g:
                # Collect evidence
                from evidence_ledger import get_evidence_ledger
                ledger = get_evidence_ledger(graph.id)
                evidence = ledger.to_dict()

                # Build result from completed nodes
                outputs = []
                ...
                result = {
                    "success": g.status == "completed",
                    "output": "\n".join(outputs) if outputs else f"Mission {g.status}",
                    "mission_id": graph.id,
                    "status": g.status,
                    "progress": g.progress,
                    "nodes_total": len(g.nodes),
                    "nodes_completed": sum(1 for n in g.nodes.values() if n.status == "completed"),
                    "evidence": evidence,
    ...
    try:
        ...
        ledger.record_action(
            action_type="execute",
            description=f"Executed: {goal[:80]}",
            agent_id="do_anything",
    ...
    result["duration_seconds"] = round(_time.time() - _start, 2)
    return JSONResponse(result)
