...
INTEGRITY: Each record is hash-chained to the previous record,
creating a tamper-evident audit trail. The chain can be verified
after mission completion to prove no evidence was altered.
"""
...
def _compute_hash(data: dict, previous_hash: str = "") -> str:
    """Compute SHA-256 hash of record data + previous hash (hash chain)."""
...
class ActionRecord:
    ...
    success: bool = True
    error: str = ""
    hash: str = ""           # SHA-256 hash of this record (hash-chained)
    prev_hash: str = ""      # Hash of previous record in chain
...
class EvidenceLedger:
    ...
    INTEGRITY: All records are hash-chained (SHA-256). Each record's
    hash includes the previous record's hash, creating a tamper-evident
    chain. Call verify_chain() after mission to validate integrity.
    """
    ...
    def __init__(self, mission_id: str):
        ...
        self._obs_counter = 0
        self._ver_counter = 0
        self._chain_hash = hashlib.sha256(f"mission:{mission_id}".encode()).hexdigest()
    ...
    def _chain_record(self, record: dict) -> str:
        """Hash-chain a record: hash = SHA-256(prev_hash + canonical_json(record))."""
        record_hash = _compute_hash(record, self._chain_hash)
    ...
    def record_action(self, action_type: str, description: str,
        ...
        # Hash-chain the record
        action.hash = self._chain_record(action.to_dict())
    ...
    def record_observation(self, action_id: str, observation_type: str,
        ...
        obs.hash = self._chain_record(obs.to_dict())
    ...
    def record_verification(self, action_id: str, observation_id: str,
        ...
        ver.hash = self._chain_record(ver.to_dict())
    ...
    def verify_chain(self) -> bool:
        ...
        """
        chain = hashlib.sha256(f"mission:{self.mission_id}".encode()).hexdigest()
        all_records = []
        for a in self.actions:
            all_records.append(("action", a))
        for o in self.observations:
            all_records.append(("observation", o))
        for v in self.verifications:
            all_records.append(("verification", v))
        all_records.sort(key=lambda x: x[1].sequence)

        for record_type, record in all_records:
            d = record.to_dict()
            stored_hash = d.pop("hash", "")
            stored_prev = d.pop("prev_hash", "")
            if stored_prev != chain:
                return False
            computed = _compute_hash(d, chain)
